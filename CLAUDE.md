...
- push / merge は Claude が実行してよい（ユーザー了承済み）。作業をコミットしたらそのまま `main` へ push し、結果（push 出力と `git log origin/main`）を確認して報告する
- force push・履歴の書き換え・ブランチ削除など取り返しのつかない操作は、実行前にユーザーに確認する
